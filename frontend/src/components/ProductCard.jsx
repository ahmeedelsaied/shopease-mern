    ...
    <Card variant="product" className="group flex h-full flex-col overflow-hidden">
      <div className="relative overflow-hidden bg-surface-container">
        <Link to={`/products/${productId}`} className="block" aria-label={`View details for ${product.name}`}>
          <ImageWithSkeleton src={product.image} alt={product.name} loading="lazy" decoding="async" wrapperClassName="relative aspect-[4/4.7]" className="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.045]" />
